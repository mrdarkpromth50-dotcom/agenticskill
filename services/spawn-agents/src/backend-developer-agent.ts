...
export class BackendDeveloperAgent extends BaseSpawnAgent {
  ...
  public async executeTask(): Promise<AgentExecutionResult> {
    ...
    try {
      ...
      const apiRequirements = await this.useLLM(`Analyze the following API requirements and propose a high-level design for a backend service using ${this.config.skills?.[0] || 'Node.js'}:\n\n${this.taskPayload.description}`);
      ...
      const apiCode = await this.useLLM(`Implement the API endpoints for the backend service using ${this.config.skills?.[0] || 'Node.js'} and Express. Use the following database schema. Provide only the code.\n\nDatabase Schema:\n${dbSchema}\n\nAPI Requirements:\n${apiRequirements}`);
