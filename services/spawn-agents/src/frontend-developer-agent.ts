...
export class FrontendDeveloperAgent extends BaseSpawnAgent {
  ...
  public async executeTask(): Promise<AgentExecutionResult> {
    ...
    try {
      ...
      const analysis = await this.useLLM(`Analyze the following UI requirements and propose a component structure for a web application using ${this.config.skills?.[0] || 'React'}:\n\n${this.taskPayload.description}`);
      ...
      const componentStructure = await this.useLLM(`Based on this UI analysis, design a detailed component structure, including component names, props, and hierarchy, for a ${this.config.skills?.[0] || 'React'} application:\n\n${analysis}`);
      ...
      const uiCode = await this.useLLM(`Implement the UI code for the following component structure using ${this.config.skills?.[0] || 'React'} and TailwindCSS. Focus on a clean, modular design. Provide only the code.\n\n${componentStructure}`);
